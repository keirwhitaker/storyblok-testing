...
CLEAN_IMAGES = false # change to true to delete unused images
...
def download_image(url, title, index = nil)
  ...
  # ⚡️ Skip auth for signed Notion URLs (X-Amz)
  req["Authorization"] = "Bearer #{NOTION_TOKEN}" unless url.include?("X-Amz-") || url.include?("prod-files-secure.s3")
...
def generate_markdown(entries)
  ...
  Dir
    .glob("_places/*.md")
    .each do |file|
      filename_slug = File.basename(file, ".md")
      unless notion_slugs.include?(filename_slug)
        File.delete(file)
        puts "🗑️  Deleted stale file: #{file}"
      end
    end
  ...
  # -----------------------------------------------------------------
  if CLEAN_IMAGES
    puts "🧹 Checking for unused images..."
    used_images = []
    ...
    entries.each do |item|
      ...
      inline_images = []
      blocks_to_markdown(body_blocks, inline_images)
      all_images = (property_images + inline_images).uniq
      all_images.each_with_index do |_url, i|
        pattern = "#{clean_filename(title)}-#{i + 1}-*"
        used_images.concat(Dir.glob("assets/uploads/#{pattern}"))
      end
    end
    ...
    Dir
      .glob("assets/uploads/*")
      .each do |file|
        unless used_images.include?(file)
          File.delete(file)
          puts "🗑️  Deleted unused image: #{file}"
        end
      end
  end

  # -----------------------------------------------------------------
  # Create/update from Notion
  # -----------------------------------------------------------------
  entries.each do |item|
    props = item["properties"]
    title = extract_property_value(props["Name"]) || "Untitled"
    ...
    props.each do |key, value|
      ...
      next if %w[Name Gallery Image Photos].include?(key)
    ...
    # Ordered keys for front matter
    ordered_keys = %w[title neighbourhood category tags short_description address longitude latitude website instagram price gallery]
    ...
    ordered_keys.each do |key|
      value =
        ...
        when "title"
          title
        when "gallery"
          front_matter_gallery
        else
          additional_fields[key]
        end
      front_matter[key] = value if value && !(value.respond_to?(:empty?) && value.empty?)
    end

    front_matter["layout"] = "place"
    front_matter["last_synced"] = Time.now.utc.iso8601

    (extra_keys = additional_fields.keys - ordered_keys).each { |key| front_matter[key] = additional_fields[key] }

    # Write YAML safely (quote risky strings)
    markdown = +"---\n"
    front_matter.each do |k, v|
      if v.is_a?(Array)
        ...
        if v.is_a?(String) && (k == "address" || v.match?(/[,:@&]/))
          markdown << "#{k}: \"#{v.gsub('"', '\"')}\"\n"
        else
          markdown << "#{k}: #{v}\n"
        end
